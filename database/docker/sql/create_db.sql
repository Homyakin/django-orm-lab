...
CREATE TABLE IF NOT EXISTS `storage`.`product` (
  ...
  `rack_position` VARCHAR(45) NOT NULL,
  PRIMARY KEY (`id`),
  INDEX `fk_product_contract1_idx` (`contract_number` ASC) VISIBLE,
  INDEX `fk_product_rack1_idx` (`rack_number` ASC) VISIBLE,
  UNIQUE INDEX `UNIQUE_POSITION` (`rack_number` ASC, `rack_position` ASC) VISIBLE,
  CONSTRAINT `fk_product_contract1`
    FOREIGN KEY (`contract_number`)
    REFERENCES `storage`.`contract` (`number`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_product_rack1`

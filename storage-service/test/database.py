...
if __name__ == '__main__':
    ...
    for i in find_contract:
        print(i.to_dict())

    space = Space.create(name='testSpace', useful_volume=100, temperature=0, humidity=0)
    ...
    for i in range(5):
        products.append(Product.create(height=1, width=1, length=1, weight=1,
                                       ...
                                       rack_number=rack1.number, rack_position=i + 1
                                       )
                        )
        products.append(Product.create(height=1, width=1, length=1, weight=1,
                                       date_of_receipt=datetime.date.today(), min_temperature=0, max_temperature=0,
                                       min_humidity=0, max_humidity=0, contract_number=contract2.number,
                                       rack_number=rack2.number, rack_position=i + 1
                                       )
                        )
